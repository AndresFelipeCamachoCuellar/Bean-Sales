...
- **⚠️ HTTPS/SSL pendiente (bloqueante en móvil):** el sitio responde por HTTP pero **no tiene certificado SSL emitido**; Chrome móvil fuerza `https://` y da `ERR_CONNECTION_RESET`. `Program.cs` ya tiene `UseHsts()` + `UseHttpsRedirection()`. **Fix: emitir el certificado gratuito Let's Encrypt en el panel de MonsterASP → sección "HTTPS / SSL certificates"** para `kingbean.runasp.net`. Es configuración del host, no código.
